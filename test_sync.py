...
def test_run_multi_pick_uses_full_segment_numbering(monkeypatch, capsys):
    """--pick uses the FULL noteworthy-segment numbering (matching the GUI/timeline), not
    a covered-only renumbering; picked segments with no footage are dropped + warned."""
    import main, clip_sources
    from datetime import datetime, timezone
    from types import SimpleNamespace
    from config import Config
    def _s(path, base):
        return clip_sources.ClipSource(path=path, base_offset=base, duration=10.0, width=1920,
                                       height=1080, creation_time=datetime(2026,7,5,tzinfo=timezone.utc),
                                       offset_source="metadata")
    monkeypatch.setattr("clip_sources.build_clip_sources", lambda *a, **k: [_s("A.mp4", 0.0)])  # covers ride 0..10
    # 3 noteworthy segments in ride order; #3 has no footage (ride 50..54, outside the clip)
    monkeypatch.setattr(main, "_segment_ranges_multi", lambda a, g, c: [
        (2.0, 6.0, "Seg1", {}), (3.0, 8.0, "Seg3", {}), (50.0, 54.0, "Geen video", {})])
    args = SimpleNamespace(video=["A.mp4", "B.mp4"], mode="segments", dry_run=True,
                           pick="2,3", strava_activity_id=None, output="o.mp4",
                           output_height="1080", music=None, gpx=None)
    rc = main._run_multi(args, Config(), object(), None, False, "o.mp4")
    out = capsys.readouterr()
    assert rc == 0
    assert "Segment-highlights (3)" in out.out               # ALL noteworthy segments numbered
    assert "geen video voor: Geen video" in out.err          # picked #3 has no footage -> dropped
    assert "(overgeslagen)" in out.out                       # #1 Seg1 not picked
    assert "(geen video)" in out.out                         # #3 picked but uncovered
